...
//                ['radio', 'prohibit_back', '是否禁止撤单','', ['禁止撤单', '允许撤单']],
    ...
    public function borrow()
    {
        $map = $this->getMap();
        // 获取子账户风控信息数据
        $info = $data_list = Db::name('stock_borrow b')
          ->where($map)
          ->where('a.sub_account', '<>', '')->order('verify_time desc')
          ->view('stock_subaccount a', 'sub_account', 'a.id = b.stock_subaccount_id', 'left')
          ->view('stock_position p ', 'stock_count,gupiao_code,gupiao_name,count,beizhu,ck_price,buy_average_price,ck_profit_price,ck_profit,profit_rate', 'p.sub_id = a.id', 'left')
          ->field('b.member_id,b.type,b.status,b.deposit_money,b.multiple,b.init_money,b.borrow_money,b.borrow_duration,b.loss_warn,b.loss_close,b.position,b.end_time,b.verify_time,b.borrow_interest')
          ->paginate()
        ...
        $page = $info->render();
        return ZBuilder::make('table')
          ->setPageTitle('子账户管理股票')// 设置页面标题
          ->setTableName('stock_borrow')// 设置数据表名
          ->addOrder('id,verify_time')// 添加排序
          ->setSearch(['sub_account' => '子账号', 'gupiao_code' => '股票代码', 'member_id' => '用户id'])// 设置搜索参数
          ->hideCheckbox()
          ...
          ->addColumns([
            ...
            ['ck_profit_price', '盈亏成本价'],
            ['ck_profit', '浮动盈亏'],
            ['profit_rate', '盈亏比例'],
            ['type', '类型', 'text', '', ['5' => '免费配资', '1' => '按天配资', '2' => '按周配资', '3' => '按月配资', '4' => '试用配资']],
            ['status', '类型', 'text', '', ['-1' => '待审核', '0' => '未通过', '1' => '使用中', '2' => '已结束', '3' => '已逾期']],
            ['deposit_money', '保证金'],
            ['multiple', '杠杆倍数'],
            ['init_money', '股票初始资金'],
            ['borrow_money', '融资金额'],
            ['borrow_duration', '配资时长'],
            ['borrow_interest', '配资管理费'],
            ['loss_warn', '预警线'],
            ['loss_close', '平仓线'],
            ['position', '仓位限制'],
            ['end_time', '结束时间'], ['verify_time', '添加时间', 'datetime']
          ])
