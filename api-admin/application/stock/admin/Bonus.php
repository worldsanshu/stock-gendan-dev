...
class Bonus extends Admin
{
    ...
    public function index()
    {
        ...
        $map = $this->getMap();
        // 数据列表
        $data_list = Db::name("stock_bonus")->where($map)->order('id desc')->paginate();
        // 分页数据
        $page = $data_list->render();



        $data_list = empty($data_list) ? $data_list : $data_list->toArray()['data'];
        foreach ($data_list as $k => $v) {
            $data_list[$k]['bonus'] = ($data_list[$k]['zuan'] ?? '--') . "|" . ($data_list[$k]['song'] ?? '--') . "|" . ($data_list[$k]['bonus'] ?? '--');
        }
        $btn_detail = [
          'title' => '符合条件股票详情',
