<?php
// +----------------------------------------------------------------------
// | 版权所有 2017~2018 蔡伟明 <314013107@qq.com>科技有限公司 [ http://www.luranjiayi.com ]
// +----------------------------------------------------------------------
...
class Renewal extends Model
{
    ...
    {
        $data_list = self::view('stock_renewal a', true)
          ->view("stock_borrow b", 'multiple,init_money,borrow_money,stock_subaccount_id,deposit_money,borrow_duration as b_duration,type,end_time', 'a.borrow_id=b.id', 'left')
          ->view('stock_subaccount_risk ssr', 'loss_warn, loss_close,renewal', 'b.stock_subaccount_id=ssr.stock_subaccount_id', 'left')
          ->view("member m", 'mobile,name,email', 'a.uid=m.id', 'left')
          ->where('a.status', '<>', 0)
          ->where('a.type', '=', 2)
    ...
    public function saveRenewal($id, $status, $info)
    {
        ...
        $recordinfo = Db::name('money_record')->where('sn', $info['id'])->find();
        Db::startTrans();
        if ($status == 1) {//审核通过
            ...
            $sdata['end_time'] = self::getAddTime($info['type'], $info['end_time'], $info['borrow_duration']);
            ...
            $adata['status'] = 1;
            $obj = ['affect' => 0, 'account' => $minfo['account'],
              'affect_activity' => 0, 'activity_account' => $minfo['activity_account'], 'sn' => ''];
            $mmoney['freeze'] = $minfo['freeze'] - abs($recordinfo['affect']);
            $mmoney['freeze_activity'] = $minfo['freeze_activity'] - abs($recordinfo['affect_activity']);
        } else {//审核不通过
            $mmoney['account'] = $minfo['account'] + abs($recordinfo['affect']);
            $mmoney['activity_account'] = $minfo['activity_account'] + abs($recordinfo['affect_activity']);
            $mmoney['freeze'] = $minfo['freeze'] - abs($recordinfo['affect']);
            $mmoney['freeze_activity'] = $minfo['freeze_activity'] - abs($recordinfo['affect_activity']);
            $msg = '审核未通过，返回冻结服务费';
            $type = 29;
            ...
            $risk_res = true;
            $adata['status'] = 2;
            $obj = ['affect' => abs($recordinfo['affect']), 'account' => $mmoney['account'],
              'affect_activity' => abs($recordinfo['affect_activity']), 'activity_account' => $mmoney['activity_account'], 'sn' => ''];
        }
        #该处存在逻辑未处理，如果是有活动金的扣除。这里是需要退出的。
        //退冻结费用
        $money_res = MoneyModel::money_up($info['uid'], $mmoney);
        $record = new RecordModel();
        $record_res = $record->saveData($info['uid'], $info['borrow_fee'], $mmoney['account'], $type, $msg, '', '', $obj);
        ...
        if ($record_res && $money_res && $renewal_res && $borrow_res && $risk_res) {
            ...
            $MemberMessageModel->addInnerMsg($info['uid'], '续期审核通知', $msg, 7, $info['borrow_id']);//站内信
            Db::commit();
...
//            $record_res = $record->saveData($info['uid'], $affect = $info['borrow_fee'], $mmoney['account'], $type, $msg);
            $obj = ['affect' => $info['borrow_fee'], 'account' => $mmoney['account'], 'affect_activity' => 0, 'activity_account' => $minfo['activity_account'], 'sn' => ''];
            $record_res = $record->saveData($info['uid'], $info['borrow_fee'], $mmoney['account'], $type, $msg, '', '', $obj);
        } else {
            $record_res = true;
            $money_res = true;
        ...
        if ($record_res && $money_res && $stop_res && $settlement_res) {
            ...
            $MemberMessageModel = new MemberMessageModel();
            $MemberMessageModel->addInnerMsg($info['uid'], '提前终止配资审核通知', $msg, 4);//站内信
            Db::commit();
    ...
    public function settlementFinancing($surplus_money, $addmoney = 0, $drawprofit = 0, $id)
    {
        ...
        if ($binfo) {
            ...
            if ($addmoney == 0) {
                $init_money = $binfo['init_money'];
            } else {
                $init_money = $binfo['init_money'] - $addmoney;
            }
            $init_money = $binfo['init_money'];
            #盈利金额=剩余配资金额-（配资总金额-累计追加保证金）+（累计提取盈利-累计追加保证金）
            $money = round($surplus_money - $init_money + $resu, 2);//盈利金额
            printlog($init_money, "配资总金额：", "aaad");
            printlog($surplus_money, "剩余配资金额：", "aaad");
            printlog($drawprofit, "提盈：", "aaad");
            printlog($addmoney, "追加的保证金：", "aaad");
            $money = round($surplus_money - $init_money + $drawprofit - $addmoney, 2);//盈利金额
            printlog($money, "money：", "aaad");
            ...
            if ($money > 0) {//盈利
                ...
                if ($binfo['type'] == 5) {//免费配资
                    ...
                    $affect_money2_tmd = $money * $freeSet[2] / 100;
                    $share_rate = $freeSet[2];
                } else {
                    $affect_money2 = ($money - $drawprofit + $addmoney);
                    $str = $affect_money2 . "（变动金额）=" . $money . "(盈利金额）-" . $drawprofit . "(累计提赢)+" . $addmoney . "(累计追加保证金)";
                    printlog($str, "money：", "aaad");
                    #最后盈利部分=盈利金额-累计提取盈利+累计追加保证金
                    # $affect_money2_tmd = ($money - $drawprofit + $addmoney) / 100;
                    $affect_money2_tmd = $money / 100;
                    $share_rate = 100;
                }
                $str = $affect_money2 . "（变动金额）=" . $money . "(盈利金额）-" . $drawprofit . "(累计提赢)+" . $addmoney . "(累计追加保证金)";
                printlog($str, "money：", "aaad");
                ...
                //亏损 如果是免费配资亏损由平台承担，否则由用户承担
                $lossMoney = -$money;//转化为正值
                if (($binfo['type'] === 5) && ($lossMoney >= ($binfo['deposit_money'] + $addmoney))) {//免费配资  如果亏损大于等于保证金
                ...
                $type = 9;
                $type2 = $binfo['type'] == 4 ? 0 : 20;//试用配资，平台承担亏损、返回保证金
                $affect_money = $binfo['deposit_money'];
    ...
    public function settlementFinancing_20230808($surplus_money, $addmoney = 0, $drawprofit = 0, $id)
    {
    ...
    /**    2020-2-01 调整  调整原因：因提赢和追加保证金都有调整
     *    结算
     *    $surplus_money :剩余配资金额
     *    $addmoney 累计追加保证金
     *    $drawprofit 累计提取盈利
     ...
     *    $contents :说明
     */
    public function settlementFinancing_bak($surplus_money, $addmoney = 0, $drawprofit = 0, $id)
    {
        //$binfo = BorrowModel::getBorrowById($id,1);
        $binfo = Db::name("stock_borrow")->where(["id" => $id])->where('status', 'in', [1, 3])->find();
        if ($binfo) {
            ...
            if ($money > 0) {//盈利
                ...
                $money_tmd = ($money + $addmoney) / 100;
                if ($binfo['type'] == 5) {//体验配资
                    $msg2 = "配资使用期限结束，盈利{$money_tmd}元，盈利的{$share_rate}% {$affect_money2_tmd}元归您,盈利部分已加到活动金";
                } else {
                    $msg2 = "配资使用期限结束，盈利{$money_tmd}元，盈利的{$share_rate}% {$affect_money2_tmd}元归您";
                }
            } elseif (abs($money) < 0.001) {//持平
            ...
            $minfo = Db::name("money")->lock(true)->where(['mid' => $binfo['member_id']])->find();
            //释放保证金
            $record_res1 = $this->addMoneyLogInfo($binfo['member_id'], $minfo, $type, $affect_money, $binfo['deposit_money'], $binfo['init_money'], $msg, $binfo['type']);
            //必须重新获取会员资金信息不然上次存储的资金资金变动不起作用
            $minfo = Db::name("money")->lock(true)->where(['mid' => $binfo['member_id']])->find();
            if ($type2 === 20) {
                //将盈利（或亏损）加到会员可用余额
                $record_res2 = $this->addMoneyLogInfo($binfo['member_id'], $minfo, $type2, $affect_money2, 0, 0, $msg2, $binfo['type']);
            } else {
                $record_res2 = true;
            }
            //最终盈利金额
    ...
    private function addMoneyLogInfo($uid, $minfo, $type, $affect, $deposit_money = 0, $init_money = 0, $info = [], $p_type = '')
    {
...
//        $record_res = $record->saveData($uid, $affect, $account, $type, $info);
        $obj = ['affect' => $affect, 'account' => $account, 'affect_activity' => 0, 'activity_account' => $minfo['activity_account'], 'sn' => ''];
        $record_res = $record->saveData($uid, $affect, $account, $type, $info, '', '', $obj);
        //退保证金并添加到可用余额
        if ($p_type == 4) {
            #试用盈利部分只能加到活动金
            $mmoney['activity_account'] = $minfo['activity_account'] + $affect;;
        } else {
