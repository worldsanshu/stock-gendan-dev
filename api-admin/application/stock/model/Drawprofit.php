...
class Drawprofit extends Model
{
    ...
    public static function getDrawprofit($map, $order)
    {
        ...
        $data_list = self::view('stock_drawprofit a', true)
          ...
          ->view('stock_subaccount_money ssm', 'stock_subaccount_id,avail,available_amount', 'b.stock_subaccount_id=ssm.stock_subaccount_id', 'left')
    ...
    //存储提盈审核结果
    public function saveDrawprofit($id, $status, $info, $remark)
    {
        ...
        if ($status == 1) {//审核通过
            ...
            # print_r($submoney_info);
            # print_r($info);
//            echo "\r\n";
//echo sprintf("%.2f",$submoney_info['available_amount']*100);
//echo "\r\n";
//          echo  sprintf("%.2f",$submoney_info['avail']*100);
//            echo "\r\n";
//          echo $info['money'];
            if (round(subtixian_new($info['stock_subaccount_id']), 2) < $info['money'] / 100) {
                #新改版
                #    if ( tixian($info['stock_subaccount_id'])/100  < $info['money']) {
                return ['status' => '0', 'msg' => '可提盈金额不足，审核失败'];
            }
            if (sprintf("%.2f", $submoney_info['avail'] * 100) < $info['money']) {
        ...
        if ($drawprofit_res && $sub_res && $record_res && $money_res) {
            if ($minfo_m['mobile']) {
                ...
                if ($status == 1) {
                    ...
                    $contentarr = getconfigSms_status(['name' => 'stock_no_drawprofit_saveDrawprofit']);
                }
                $content = str_replace(array("#var#", "#order_id#", "#code#"), array($minfo_m['mobile'], $info['order_id'], $remark), $contentarr['value']);
                if ($contentarr['status'] == 1) {
                    $res = sendsms_mandao($minfo_m['mobile'], $content, 'user');
                }
            }
            $MemberMessageModel = new MemberMessageModel();
            $MemberMessageModel->addInnerMsg($info['uid'], '提取盈利审核通知', $msg, 5);//站内信
            Db::commit();
