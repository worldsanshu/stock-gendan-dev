    ...
    */
    ...
    public function saveBorrow($data)
    {
        ...
        if ($stock_borrow['borrow_interest_activity'] == 0) {
            ...
            $mmoney['k'] = "全部从余额扣除";
            //当前配资冻结金额=保证金+活动金
            $freeze = $sumMoney = ($stock_borrow['deposit_money'] + $stock_borrow['borrow_interest']);
            $borrow_interest_activity = 0;
        } else if ($stock_borrow['borrow_interest_activity'] == $stock_borrow['borrow_interest']) {
            //全部从活动金总扣除
            //当前配资冻结金额=保证金
            $freeze = $sumMoney = ($data['deposit_money']) * 100;
            $borrow_interest_activity = $data['borrow_interest'] * 100;
            $mmoney['k'] = "全部从活动金总扣除";
        } else {
            //部分从余额扣除  部分从保证金中扣除
            //当前配资冻结余额=保证金+从余额扣除部分（总利息减去从活动金总扣除部分）
            #余额支付总金额：用于加回余额
            $sumMoney = $data['deposit_money'] + $data['borrow_interest'] - $stock_borrow['borrow_interest_activity'];
            #总冻结资金:用于从用户原有的保证金总减出来
            $freeze = $data['deposit_money'] + $data['borrow_interest'];
            #用的那部分活动金
        ...
        if ($data['v_status'] == 0) {//审核未通过
            ...
            //  $mmoney['freeze2'] =$minfo['freeze'];
            $newfreeze = $minfo['freeze'] - $freeze;
            if ($newfreeze < 0) {
                //$newfreeze=0;
            }
            $mmoney['freeze'] = $newfreeze;//资金解冻，减掉冻结中的资金
            ...
            $record_res = $record->saveData($data['member_id'], $sumMoney, $mmoney['account'], 21, '配资审核未通过，解冻保证金到余额，余额：' . $hs_sumMoney . '元，活动金：' . $borrow_interest_activity . '元');
            $res = $this->where(['id' => $data['id']])->update(['status' => 0]);
...
//                }
            ...
            // 更新资金日志表信息
            $mmLogRet = $record->saveData($data['member_id'], $sumMoney, $minfo['account'], 22, '配资审核通过，解冻保证金到配资账户,保证金金额：' . $data['deposit_money']);
            //更新配资表信息
            $borrow = self::get($data['id']);
            $dataInfo['status'] = $data['v_status'];
            $dataInfo['stock_subaccount_id'] = $data['stock_subaccount_id'];
...
//        die();
        try {
            $money_res = Db::name('money')->where(['mid' => $data['member_id']])->update(['account' => $updatamone, 'freeze' => $freeze, 'freeze_activity' => $freeze_activity, 'activity_account' => $activity_account]);
            $record_res = $record->saveData($data['member_id'], -abs($need_money), $updatamone, 33, $str);
            // 验证
...
//        }
        ...
        if (empty($position_res['buy_average_price'])) {
            file_put_contents("123456789aaaa.txt", '价格异常');
            return ['status' => 0, 'message' => '股票价格异常'];
        }
        ...
        $pos_res = $Delivery->sell_m_delivery_order($stockinfo, $count, $price, $buy_average_price, $sub_id, $lid, $user, $soure, $commission, $Transfer, $Trust_no, $stamp, $avail, $amount, $model);
        if (!$pos_res) {
            Db::rollback();
            return ['status' => 0, 'message' => "交易失败"];
        }
