...
class Builder extends ZBuilder
{
   ...
   * @author 蔡伟明 <314013107@qq.com>
   */
  public function addImg($name = '', $title = '', $tips = '', $default = '', $extra_class = '')
  {
   ...
   *      数组包含：table-表名，id-主键字段，name-标题字段，list_rows-每页显示数量，search-模糊搜索字段，where-附加查询条件
   *      字符串：表示自定义查询数据的url
   * @param string $default 默认值
   * @param string $extra_attr 额外属性
   * @param string $extra_class 额外css类名
   ...
   * @author 蔡伟明 <314013107@qq.com>
   */
  public function addSelectAjax($name = '', $title = '', $tips = '', $params = [], $default = '', $extra_attr = '', $extra_class = '')
  {
    $url = url('admin/ajax/getSelectAjax');
    if (is_array($params)) {
      $token = $this->createSelectAjaxToken($params);
    } else {
      $token = '';
      $url   = $params;
    }
   ...
   * @author 蔡伟明 <314013107@qq.com>
   */
  private function createSelectAjaxToken($params = [])
  {
    // 排序
    ksort($params);
    $code        = http_build_query($params);
    $table_token = substr(sha1($code), 0, 8);
    session($table_token, $params);
    return $table_token;
  }
    ...
    if ($type != '') {
      switch ($type) {
        ...
        case 'select_ajax':
          ...
          $this->_vars['_js_init'][]   = 'select-ajax';
          break;
        case 'masked':
