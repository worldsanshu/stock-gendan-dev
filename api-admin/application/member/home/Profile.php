...
class Profile extends Common
{
    ...
    {
        return json(['data' => "/static/member/img/avatar-male.png", 'status' => 1, 'message' => '图片上传成功']);
    ...
    public function getmobliecode()
    {
        ...
        if (!check_sms_code($phone)) {
            return json(['status' => 0, 'message' => "请间隔60秒再获取验证码！"]);
        };
    ...
    public function realname()
    {
        ...
        if (!empty($post)) {
            ...
            $params = array(
              'cardNo' => $id_card,
              'realName' => $name,
              'appkey' => config('JD_appkey')
            );
            $url = config('JD_url');
            $YHVerifyIdcard = http_request($url, $params);
            $YHVerifyIdcard = json_decode($YHVerifyIdcard, true);
            if ($YHVerifyIdcard['result']['result']['isok'] == '1') {
                $res = Db::name('member')->where(['id' => $mid])->update(['name' => $name, 'id_card' => $id_card, 'id_auth' => 1, 'auth_time' => time()]);
                if ($res) {
        ...
        $this->assign('info', $datalist);
        $this->assign('url', $url);
        $this->assign("is_verified", intval($datalist['id_auth']));
