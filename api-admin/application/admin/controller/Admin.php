...
class Admin extends Common
{
    ...
    protected $documentary_array = [];
    protected $trader_documentary_array = [];
    ...
    protected function initialize()
    {
        ...
        $this->trader_documentary_array = traderDocumentaryArray();
        $this->documentary_array        = documentaryArray();
        ...
        role_auth();
        // 检查权限
        if (request()->path() != 'admin/admin/dellocationsarr') {
        ...
        if (!$this->request->isAjax()) {
            #平台上架时间
            $AddedTime = config('Added_time');
            $AddedTime = timedifference(strtotime($AddedTime), time());
            $this->assign('AddedTime', $AddedTime['d'] . '天' . $AddedTime['d'] . '小时');

            #平台充提差  扣除白名单
            $memberIdslist = $memberIds = Db::name('member')
            ...
            // 查询数据并排除指定 ID 列表的数据
//            $user_statistics = Db::name('recharge_withdrawal')
//                ->whereOr('recharge_user', 'in', $memberIds)
//                ->whereOr('withdrawal_user', 'in', $memberIds)
//                ->where((new UserService())->getAgentSql('id'))
//                ->field("SUM(`recharge`) as recharge,SUM(`withdrawal`) as withdrawal")->find();


//            提现总量
            $user_statistics['withdrawal'] = Db::name('money_withdraw')
            ...
            $Record_withdraw = $Record_withdraw / 100; //单位元
            $user_statistics['withdrawal'] = sprintf("%.2f", $user_statistics['withdrawal']/100 + $Record_withdraw);// 保留两位小数

//            充值总量
            $user_statistics['recharge'] = Db::name('money_recharge')
            ...
            $Record_recharge = $Record_recharge / 100; //单位元
            $user_statistics['recharge'] = sprintf("%.2f", $user_statistics['recharge']/100 +$Record_recharge);// 保留两位小数

            $Difference = $user_statistics['recharge'] - $user_statistics['withdrawal'];
            $Deposit_Withdrawal_Difference = 0;
            if ($Difference > 0) {
                $Deposit_Withdrawal_Difference = $Difference. '元';
            }
//            $this->assign('Deposit_Withdrawal_Difference', convertToTenThousand($user_statistics['recharge'] - $user_statistics['withdrawal']));
            $this->assign('Deposit_Withdrawal_Difference', $Deposit_Withdrawal_Difference);

            #平台累计充值
//            $this->assign('sum_recharge', convertToTenThousand($user_statistics['recharge']));
            $this->assign('sum_recharge', $user_statistics['recharge']);
            #平台累计提现
//            $this->assign('sum_withdrawal', convertToTenThousand($user_statistics['withdrawal']));
            $this->assign('sum_withdrawal', $user_statistics['withdrawal']);
            #平台用户总量
            $this->assign('sum_member', count($memberIdslist) . '个');

            #跟单总用户
            $fund_ordergs = Db::name('fund_order_gs')->whereIn('status', [1, 3, 6])->field("id")->where((new UserService())->getAgentSql('uid'))->group('uid')->select();
            $this->assign('fund_ordergs', count($fund_ordergs) . '个');
...
//            $this->assign('_location', MenuModel::getLocation('', true));
            $getLocation_new = MenuModel::getLocation_new('', true);
            if ($getLocation_new) {
                $this->assign('_location', MenuModel::getLocation_new('', true));
            } else {
                $this->error('获取不到当前节点地址，可能未添加节点;<br>联系管理添加节点');
            }
            ...
            // 获取提现待处理
            $this->assign('_withdraw', MessageModel::getMoneyCount(1));
            // 获取充值待处理
            $this->assign('_recharge', MessageModel::getMoneyCount(2));
            // 构建侧栏
            $data        = [
