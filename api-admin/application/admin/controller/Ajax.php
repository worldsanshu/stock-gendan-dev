...
class Ajax extends Common
{
    ...
    {
        if (!is_signin()) {
            $this->error('登录已失效，请重新登录', '/admin_user');
        }
        role_auth();
     ...
     * @param string $_type 操作类型：query-查询，query:append-查询分页数据，default-查询默认值
     * @param string $_default 默认值
     * @return \think\response\Json
     * @throws \think\exception\DbException
     * @author 蔡伟明 <314013107@qq.com>
     */
    public function getSelectAjax($_keyword = '', $_token = '', $_type = '', $_default = '')
    {
        // 获取参数
        $params = session($_token);
        // 主键字段
        $key = 'id';
        // 标题字段
        $title = 'name';
        ...
        if (isset($params['name']) && $params['name'] != 'text') {
            $title = $params['name'];
        }
        if ($_type == 'default') {
            // 获取默认选项
            if (isset($params['prefix']) && $params['prefix'] === true) {
                $result = Db::table($params['table'])
                    ->where($key, $_default)
                    ->value($title);
            } else {
                $result = Db::name($params['table'])
                    ->where($key, $_default)
                    ->value($title);
            }
            $option = [
            ...
            // 查询条件
            $where = [
                [$search, 'like', '%' . $_keyword . '%']
            ];
            if (isset($params['where'])) {
                $where = array_merge($where, $params['where']);
            }
            // 每页显示数量
            $listRows = isset($params['list_rows']) ? $params['list_rows'] : 15;
            if ($_keyword != '') {
                if (isset($params['prefix']) && $params['prefix'] === true) {
                    $data_list = Db::table($params['table'])
                        ->where($where)
                        ->field($field)
                        ->paginate($listRows);
                } else {
                    $data_list = Db::name($params['table'])
                        ->where($where)
                        ->field($field)
                        ->paginate($listRows);
                }
     ...
     * 获取充值提示音
     */
    public function getSeeNum()
    {
        if (!is_signin()) {
            $this->error('请先登录');
        }
//        充值记录
        $recharge = Db::name('money_recharge')->where('status', 0)->count();
//        提现记录
        $withdraw = Db::name('money_withdraw')->where('status', 0)->count();

        $result['code'] = 0;
//        有充值和提现
        if ($recharge >0 && $withdraw >0) {
            $result['code'] = 1;
            $result['data'] = '/uploads/voices/cztx.mp3';
        }else if ($recharge >0 && $withdraw <=0) {
//            只有充值
            $result['code'] = 1;
            $result['data'] = '/uploads/voices/cz.mp3';
        }else if ($recharge <=0 && $withdraw >0) {
//            只有提现
            $result['code'] = 1;
            $result['data'] = '/uploads/voices/tx.mp3';
        }
        return $result;
    }
}