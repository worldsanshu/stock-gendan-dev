...
class Menu extends Model
{
     ...
     * @author 蔡伟明 <314013107@qq.com>
     */
    public static function getLocation_new($id = '', $del_last_url = false, $check = true)
    {
        $model = request()->module();
        $controller = request()->controller();
        $action = request()->action();
...
//        return $location;
        $result = [
            'location' =>$location,
            'locationsArray' =>self::getLocationNew($id),
        ];
        return $result;
    }
    ...
    /**
     * Desc : 获取面包屑的tab
     * Date : 2024-06-28 03:24
     *
     * @param $id
     *
     * @return array|mixed
     */
    public static function getLocationNew($id = '')
    {
        $model = request()->module();
        $controller = request()->controller();
        $action = request()->action();
        ...
        if (!$locationsArray) {
            $locationsArray = []; // 如果缓存不存在，则初始化为空数组
        }

        // 将当前位置数组的最后一个节点添加到历史记录中（如果它不存在）
        if (!empty($location)) {
            $lastNode = end($location);
            $lastNodeId = $lastNode['id']; // 假设每个节点都有一个唯一的ID

            // 检查节点的ID是否已存在于历史记录中
            $isAlreadyInHistory = false;
            foreach ($locationsArray as $existingNode) {
                if ($existingNode['id'] == $lastNodeId) {
                    $isAlreadyInHistory = true;
                    break;
                }
            }
            // 如果节点不在历史记录中，则将其添加
            if (!$isAlreadyInHistory && $lastNodeId !='70' && $lastNodeId !='1300') {
                if ($model == 'admin' && $controller == 'Index' && $action == 'indexdata') { //首页嵌套路径不要了
                   return;
                }
                $lastNode['url_value'] = $model . '/' . $controller . '/' . $action;
                $locationsArray[] = $lastNode; // 将最后一个节点添加到历史记录数组
                // 将更新后的历史记录数组存回缓存
                cache('_history'.UID, $locationsArray);
            }
        }

// 返回当前请求的位置信息（注意：这里您可能想要返回$location而不是$locationsArray）
// 如果您想同时返回位置信息和历史记录，可以考虑返回一个关联数组
