    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            for ($i = $num - 1; $i >= 0; $i--) {
                ...
                //打乱数组顺序
                shuffle($numArr);
                $code = implode('', array_slice($numArr, 0, $lenth));
//                $str = substr(md5(uniqid()),0,$lenth);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            // 表单数据
            $data = input();
            $apk_download_link = AttachmentModel::getFilePath($data['apk']);
            $ios_download_link = AttachmentModel::getFilePath($data['ios']);
            $update_data['ios_download_link'] = $ios_download_link;
            $update_data['apk_download_link'] = $apk_download_link;
            Member::where('agent_code', $data['code'])->update($update_data);
            // 验证
