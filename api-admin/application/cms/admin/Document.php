...
class Document extends Admin
{
     ...
     * @author 蔡伟明 <314013107@qq.com>
     */
    public function add($cid = 0, $model = '', $language = 'cn')
    {
        // 保存文档数据
        if ($this->request->isAjax()) {
            $DocumentModel = new DocumentModel();
            $this->Logs('add', ['input' => input(), 'is_push' => input('is_push')], 'document');
        ...
        if ($cid > 0) {
            ...
            $extra_field = [
              ...
              ['name' => 'language', 'title' => '所属语言', 'type' => 'static', 'value' => $language],


              ['name' => 'model', 'type' => 'text', 'value' => $column['model']]
            ];
...
//        $fields = array_merge($extra_field, $fields);
            ...
            return ZBuilder::make('form')
              ->setFormItems($fields)
//              ->setFormItems($extra_field)
              ->addFormItems([ // 批量添加表单项
                ['hidden', 'cid',$column['name']],
                ['hidden', 'language',$language],
                ['hidden', 'model', $column['model']],
                ['hidden', 'cid', $cid],

              ])
//              ->addRadio('is_push', '是否推送消息', '', [1 => '是', 0 => '否'])
        ->hideBtn('back')
              ->fetch();
        }
        ...
        return ZBuilder::make('form')
          ...
          ->addFormItem('select', 'language', '选择语言', '请选择语言', $language_list)
    ...
    public function edit($id = null, $model = '')
    {
        ...
        $extra_field = [
          ...
          ['name' => 'language', 'title' => '所属语言', 'type' => 'select', 'options' => $language_list],
