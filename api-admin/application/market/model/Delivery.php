...
class Delivery extends Model
{
    ...
    public function get_delivery_order($sub_id, $beginday = "", $endday = "")
    {
        if (empty($endday)) {
            ...
            $endday = strtotime($endday) + 86400;
        }
        if (empty($beginday)) {
            $beginday = strtotime(date("Y-m-d", time()));
        } else {
    ...
    public function sell_m_delivery_order($stockinfo, $count, $price, $buy_average_price, $sub_id, $lid, $user, $soure, $commission, $Transfer, $Trust_no, $stamp, $avail, $amount, $model, $sys = 0, $fees = 0)
    {
        ...
        $data[0]['deal_price'] = $price;  //成交价格
        $data[0]['buy_price'] = $buy_average_price;  //买入价格
        $data[0]['sys'] = $sys;  //卖出类型
        $data[0]['volume'] = $count;
    ...
    public function add_m_delivery_order($stockinfo, $count, $price, $sub_id, $lid, $user, $soure, $commission, $Transfer, $Trust_no, $avail, $amount, $model, $fees = 0)
    {
        ...
        $data[0]['deal_price'] = $price;  //买入价格
        //$data[0]['deal_price'] = $stockinfo["current_price"];  //当前价格
        $data[0]['volume'] = $count;
        $data[0]['amount'] = $amount;
