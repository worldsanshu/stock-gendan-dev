...
class StockMarketWeekK extends Model
{
    // 设置当前模型对应的完整数据表名称stock_market_bat
    protected $table = '__STOCK_MARKET_WEEK_K__';
    protected $pk = 'id';
    // 自动写入时间戳
     ...
     * 批量存储和更新minute_k记录
     */
    public function action_market_week_k($code, $data)
    {
        Logs::log('action_market_week_k', ['data' => $data], 'StockMarketMonthK');
        $info = Db::name('stock_market_week_k')->where(['code' => $code])->find();
        if (!$info && $data) {
            foreach ($data as $k => $v) {
                $data[$k]['code'] = $code;
            }
            ...
            $list = Db::name('stock_market_week_k')->where(['code' => $code])->select();
            foreach ($data as $k => $v) {
                $data[$k]['id'] = $list[$k]['id'];
                $data[$k]['code'] = $code;
            }
