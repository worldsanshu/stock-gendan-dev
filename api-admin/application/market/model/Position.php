...
class Position extends Model
{
     ...
     * $sub_id 子账号
     */
    public function get_position($sub_id = "", $page = '', $order = '', $listRows = '')
    {
        if (!empty($sub_id)) {
            if (is_array($sub_id)) {
        ...
        if (count($res) === 1) {
            foreach ($res as $k => $v) {
                ...
                $res[$k]['ck_profit'] = round(($info["current_price"] - $v['ck_price_new']) * $v['stock_count'], 2);//参考浮动盈亏
                $res[$k]['profit_rate'] = round($res[$k]['ck_profit'] / ($v['ck_price_new'] * $v['stock_count']) * 100, 2);//盈亏比例
                #老版本
            ...
            foreach ($res as $i => $vl) {
                $data_code_arr[] = $vl['gupiao_code'];
            }
            //去重
            $code_arr = array_unique($data_code_arr);
            $stockMap = array();
            foreach (array_chunk($code_arr, 50) as $key => $val) {
                $scode[$key] = '';
                foreach ($val as $k => $v) {
                    if ($scode[$key] != "") {
                        $scode[$key] .= ",";
                ...
                $stockMap = array_merge($stockMap, z_market_bat($scode[$key]));
            }
            $stockMap_arr = array();
            foreach ($stockMap as $kc => $vc) {
                $stockMap_arr[$vc['code']] = $vc;
            }
            foreach ($res as $ky => $va) {
                $res[$ky]['now_price'] = $stockMap_arr[$va['gupiao_code']]['current_price'];
                $res[$ky]['code'] = $res[$ky]["gupiao_code"];
                $res[$ky]['name'] = $res[$ky]["gupiao_name"];
                $res[$ky]['market_value'] = $stockMap_arr[$va['gupiao_code']]['current_price'] * $va['stock_count'];//最新市值
                $res[$ky]['ck_profit'] = round(($stockMap_arr[$va['gupiao_code']]['current_price'] - $va['ck_price_new']) * $va['stock_count'], 2);//参考浮动盈亏
                $res[$ky]['profit_rate'] = round($res[$ky]['ck_profit'] / ($va['ck_price_new'] * $va['stock_count']) * 100, 2);//盈亏比例
                #老版本
                #  $res[$ky]['ck_profit'] = round(($stockMap_arr[$va['gupiao_code']]['current_price'] - $va['buy_average_price']) * $va['stock_count'], 2);//参考浮动盈亏
                #   $res[$ky]['profit_rate'] = round($res[$ky]['ck_profit'] / ($va['buy_average_price'] * $va['stock_count']) * 100, 2);//盈亏比例
            }
        }
        return $res;
    }

    public function get_positions($sub_id = "")
    {
        if (!empty($sub_id)) {
            $res = Db::name('stock_position')
              ->where(['sub_id' => $sub_id])
        ...
        } elseif (count($res) >= 2 && count($res) < 160) {
            $code = "";
            foreach ($res as $k => $v) {
        ...
        if (count($res) === 1) {
            foreach ($res as $k => $v) {
                ...
                $res_d1 = Db::name('stock_delivery_order')
                  ...
                  ->sum('volume');
                $res[$k]['canbuy_count'] = $res[$k]['stock_count'] - $res_d1;
    ...
    public function get_canbuy_count($sub_id, $code)
    {
        ...
        $res_d1 = Db::name('stock_delivery_order')
          ...
          ->where(['business_name' => '证券买入'])
          ->sum('volume');
        $res['canbuy_count'] = $res['canbuy_count'] - $res_d1;
    ...
    public function sell_m_position($stockinfo, $count, $price, $sub_id, $model = 2, $Trust_no, $commission, $Transfer, $stamp)
    {
        ...
        if ($canbuy_count > 0) {
            ...
            if ($model == 2) {
                ...
                $data['canbuy_count'] = $canbuy_count;
                $data['ck_price'] = $buy_average_price;//参考成本价
                $data['buy_average_price'] = $buy_average_price;//买入均价
                $data['ck_profit_price'] = $buy_average_price;//参考盈亏成本价
                ...
                $data['ck_profit'] = round(($stockinfo["current_price"] - $buy_average_price) * $canbuy_count, 2);//参考浮动盈亏
                $data['profit_rate'] = round($data['ck_profit'] / ($buy_average_price * $canbuy_count) * 100, 2);//盈亏比例
            }
    ...
    public function add_m_position($stockinfo, $count, $sub_id, $lid, $user, $soure, $ck_price, $model, $Trust_no)
    {
        ...
        if (empty($position_res)) {
            if ($model == 2) {
                ...
                $data[0]['ck_price_new'] = $ck_price;//参考成本价
...
//                $data[0]['type'] = $stockinfo["exchange_code"];//帐号类别
            ...
            if ($model == 2) {
                ...
                $data[0]['gupiao_name'] = $stockinfo["name"];
                $data[0]['count'] = $counts;
                $data[0]['stock_count'] = $stock_count;
                $data[0]['canbuy_count'] = $canbuy_count;
