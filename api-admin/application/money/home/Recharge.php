...
class Recharge extends Common
{
                ...
                if ($type == 'transfer' || $type == '扫码线下充值') {
                    ...
                    switch ($type) {
                        case "debao":
                            ...
                            break;
                        case "alipay":
                            ajaxmsg('正在处理，请稍后...', 1, url('alipay') . '?order=' . $order_no);
                            break;
                    }
                }
            } else {
    ...
    {
        $trade_no = input('order');
        $orders = Db::name('money_recharge')->where('order_no', $trade_no)->find();
        $order = [
          'out_trade_no' => $trade_no,
          'total_amount' => bcdiv($orders['money'], 100, 2),
          'subject' => '浩源收款',
        ];
