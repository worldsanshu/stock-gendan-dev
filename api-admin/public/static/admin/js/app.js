...
var App = function () {
    ...
    var uiHelperSelectAjax = function () {
        jQuery('.js-select-ajax').each(function () {
            var $select2 = jQuery(this);
            var $width = $select2.data('width') || '100%';
            var $url = $select2.data('url') || '';
            var $token = $select2.data('token') || '';
            var $value = $select2.data('value') || '';
            var $list_rows = $select2.data('list_rows') || 15;
            $select2.select2({
                width: $width, //设置下拉框的宽度
                language: "zh-CN",
                ajax: {
                    ...
                    processResults: function (data, params) {
                        ...
                        return {
                            results: data.data,
                            pagination: {
                                more: (params.page * $list_rows) < data.total
                            }
                        };
                    },
                    cache: true
            ...
            // 获取默认选项
            if ($value !== '') {
                $.post($url, {_type: 'default', _token: $token, _default: $value}, function (res) {
                    if (res.text != null) {
                        var newOption = new Option(res.text, res.id, false, false);
                        $select2.append(newOption).val($value).trigger('change');
                    }
                });
            }
        });
