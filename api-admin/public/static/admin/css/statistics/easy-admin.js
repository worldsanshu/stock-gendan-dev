    ...
    var admin = {
        ...
        table: {
            render: function (options) {
                ...
                options.limits = options.limits || [20, 30, 50, 100, 300, 500, 1000];
            ...
            renderSearch: function (cols, elem, tableId) {
                ...
                $.each(cols, function (i, d) {
                    ...
                    d.searchTitle = d.searchTitle || false;
                    ...
                    if (d.field !== false && d.search !== false) {
                        if (!d.searchTitle) {
                            d.searchTitle = d.title;
                        }
                        switch (d.search) {
                            case true:
                                formHtml += '\t<div class="layui-form-item layui-inline">\n' +
                                    '<label class="layui-form-label">' + d.searchTitle + '</label>\n' +
        ...
        listen: function (preposeCallback, ok, no, ex) {
            ...
            $('body').on('click', '[data-table-export]', function () {
                ...
                let schPar = 'filter=' + JSON.stringify(formatData.formatFilter) + '&' + 'op=' + encodeURI(JSON.stringify(formatData.formatOp));
                // 使用encodeURI编码, 改为如下
                //let schPar='filter='+encodeURI(JSON.stringify(formatData.formatFilter))+'&'+'op='+encodeURI(JSON.stringify(formatData.formatOp));
